...
rule params:
    output:
        "output/{sample}/params.txt",
    log:
        "output/{sample}/log/params.log",
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    params:
...
if phased:
    ...
    rule starAlignment:
        ...
        log:
            "output/{sample}/log/starAlignment.log",
        shell:
            """
    ...
    rule waspFilter:
        input:
            "output/{sample}/0_alignment/starAligned.sortedByCoord.out.bam",
        output:
            "output/{sample}/0_alignment/starAligned.waspFilter.bam",
        container:
            "docker://quay.io/biocontainers/samtools:1.16.1--h6899075_1"
        log:
            "output/{sample}/log/waspFilter.log",
        shell:
            """
...
rule rsem:
    input:
        "output/{sample}/0_alignment/starAligned.toTranscriptome.out.bam",
    output:
        "output/{sample}/0_alignment/star.genes.results",
    params:
        ref=config["rsemReferencePath"],
    container:
        "docker://quay.io/biocontainers/rsem:1.3.3--pl5321hecb563c_4"
    threads: config["threads"]
    log:
        "output/{sample}/log/rsem.log",
    shell:
        """
...
rule rsemExpressionMatrix:
    input:
        "output/{sample}/0_alignment/star.genes.results",
    output:
        "output/{sample}/0_alignment/expression_matrix.tsv",
    params:
        annotation="annotation/biomart_ensembl100_GRCh38.sorted.bed",
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/rsemExpressionMatrix.log",
    shell:
        """
...
rule phase_vcf_filter:
    ...
    container:
        "docker://quay.io/biocontainers/htslib:1.15--h9753748_0"
    log:
        "output/{sample}/log/phase_vcf_filter.log",
    shell:
        """
...
rule phase_vcf_index:
    ...
    log:
        "output/{sample}/log/phase_vcf_index.log",
    container:
        "docker://quay.io/biocontainers/htslib:1.15--h9753748_0"
    shell:
        "tabix {input.vcf} &> {log}"
...
if phased:
    ...
    rule rna_snv_calling:
        ...
        log:
            "output/{sample}/log/rna_snv_calling.log",
        threads: config["threads"]
        shell:
            """
...
rule pass_filt:
    ...
    container:
        "docker://quay.io/biocontainers/htslib:1.15--h9753748_0"
    log:
        "output/{sample}/log/pass_filt.log",
    shell:
...
rule rna_snv_index:
    ...
    container:
        "docker://quay.io/biocontainers/htslib:1.15--h9753748_0"
    log:
        "output/{sample}/log/rna_snv_index.log",
    shell:
        "tabix {input.vcf} &> {log}"
...
if phased:
    ...
    rule intersect:
        ...
        container:
            "docker://quay.io/biocontainers/bcftools:1.15--h0ea216a_2"
        log:
            "output/{sample}/log/intersect.log",
        shell:
            """
    ...
    rule intersect_gz:
        input:
            "output/{sample}/1_variant/rna.isec.snps.vcf",
        output:
            temp("output/{sample}/1_variant/rna.isec.snps.vcf.gz"),
        container:
            "docker://quay.io/biocontainers/htslib:1.15--h9753748_0"
        log:
            "output/{sample}/log/intersect_gz.log",
        shell:
            "bgzip {input} &> {log}"
...
rule snpSift:
    input:
        "output/{sample}/1_variant/rna.isec.snps.snpEff.vcf",
    output:
...
rule addExpression:
    ...
    output:
        "output/{sample}/2_mBASED/MBASED_expr_gene_results.txt",
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    params:
        maf=config["maf_threshold"],
    log:
        "output/{sample}/log/addExpression.log",
    shell:
        """
...
rule annotateGenes:
    ...
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/annotateGenes.log",
    shell:
        """
...
rule genomeLength:
    input:
        f"{genome_path}.fai",
    output:
        temp("output/{sample}/3_cancer/raw/genome.length"),
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/genomeLength.log",
    shell:
        """
...
rule promoterFlank:
    ...
    output:
        "output/{sample}/3_cancer/raw/promoter_annotation.bed",
    container:
        "docker://quay.io/biocontainers/bedtools:2.23.0--h5b5514e_6"
    log:
        "output/{sample}/log/promoterFlank.log",
    shell:
        """
...
rule cnv_preprocess:
    input:
        lambda w: config["samples"][w.sample]["cnv"],
    output:
        "output/{sample}/3_cancer/raw/cnv.bed",
    params:
        tumor=getTumorContent,
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/cnv_preprocess.log",
    shell:
        """
...
rule cnvIntersect:
    input:
        ...
        gene="output/{sample}/3_cancer/raw/gene_annotation.bed",
    output:
        "output/{sample}/3_cancer/intersect/cnv_intersect.bed",
    container:
        "docker://quay.io/biocontainers/bedtools:2.23.0--h5b5514e_6"
    log:
        "output/{sample}/log/cnvIntersect.log",
    shell:
        """
...
rule dmr_preprocess:
    input:
        lambda w: config["samples"][w.sample]["methyl"],
    output:
        "output/{sample}/3_cancer/raw/methyl.bed",
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/dmr_preprocess.log",
    shell:
        """
...
rule methylIntersect:
    ...
    output:
        "output/{sample}/3_cancer/intersect/methyl_intersect.bed",
    container:
        "docker://quay.io/biocontainers/bedtools:2.23.0--h5b5514e_6"
    log:
        "output/{sample}/log/methylIntersect.log",
    shell:
        """
        mkdir -p output/{wildcards.sample}/3_cancer/intersect
...
rule promoterID:
    ...
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/promoterID.log",
    shell:
        """
...
rule createNormalPromoter:
    input:
        "output/{sample}/3_cancer/tfbs/id.txt",
    output:
    ...
    container:
        "docker://quay.io/biocontainers/samtools:1.16.1--h6899075_1"
    log:
        "output/{sample}/log/createNormalPromoter.log",
    shell:
...
rule mutatePromoter:
    ...
    output:
        temp("output/{sample}/3_cancer/tfbs/allele{num}_promoter.fa"),
    container:
        "docker://quay.io/biocontainers/bcftools:1.15--h0ea216a_2"
    log:
        "output/{sample}/log/mutatePromoter_{num}.log",
    shell:
        """
...
rule scanMotif:
    input:
        "output/{sample}/3_cancer/tfbs/allele{num}_promoter.fa",
    output:
        "output/{sample}/3_cancer/tfbs/allele{num}_motif.txt",
    params:
        motifFile="annotation/HOCOMOCOv11_core_HUMAN_mono_meme_format.meme",
    container:
        "docker://quay.io/biocontainers/meme:5.4.1--py310pl5321hb021246_2"
    log:
        "output/{sample}/log/scanMotif_{num}.log",
    shell:
        """
...
rule compareTFBS:
    ...
    output:
        "output/{sample}/3_cancer/tfbs/motifDiff.tsv",
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    params:
        tf_list="annotation/human_mono_motifs.tsv",
    log:
        "output/{sample}/log/compareTFBS.log",
    shell:
        """
...
rule annotatePhase:
    input:
        lambda w: config["samples"][w.sample]["phase"],
    output:
        temp("output/{sample}/3_cancer/stopVar/phase.annotate.vcf"),
    container:
        "docker://quay.io/biocontainers/snpeff:5.0--hdfd78af_1"
    params:
        genome=config["annotationPath"]["snpEff_genomeName"],
        ...
        heapSize=config["annotationPath"]["snpEff_javaHeap"],
    log:
        "output/{sample}/log/annotatePhase.log",
    shell:
        """
...
rule oneLine:
    input:
        "output/{sample}/3_cancer/stopVar/phase.annotate.vcf",
    output:
        temp("output/{sample}/3_cancer/stopVar/phase.oneline.annotate.vcf"),
    container:
        "docker://quay.io/biocontainers/snpeff:5.0--hdfd78af_1"
    log:
        "output/{sample}/log/oneLine.log",
    shell:
        """
...
rule snpSiftPhase:
    input:
        "output/{sample}/3_cancer/stopVar/phase.oneline.annotate.vcf",
    output:
        "output/{sample}/3_cancer/stopVar/phase.annotate.tsv",
    container:
        "docker://quay.io/biocontainers/snpsift:5.1d--hdfd78af_0"
    params:
        heapSize=config["annotationPath"]["snpEff_javaHeap"],
    log:
        "output/{sample}/log/snpSiftPhase.log",
    shell:
        """
...
rule getStopMutation:
    input:
        "output/{sample}/3_cancer/stopVar/phase.annotate.tsv",
    output:
        "output/{sample}/3_cancer/stopVar/stop_variant.tsv",
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/getStopMutation.log",
    shell:
        """
...
rule getGeneSlop:
    input:
        ...
        length="output/{sample}/3_cancer/raw/genome.length",
    output:
        "output/{sample}/3_cancer/somatic/geneSlop.bed",
    container:
        "docker://quay.io/biocontainers/bedtools:2.23.0--h5b5514e_6"
    log:
        "output/{sample}/log/getGeneSlop.log",
    shell:
        """
...
rule somaticIntersect:
    ...
    output:
        "output/{sample}/3_cancer/somatic/{variant}.bed",
    container:
        "docker://quay.io/biocontainers/bedtools:2.23.0--h5b5514e_6"
    log:
        "output/{sample}/log/somatic_{variant}_intersect.log",
    shell:
        """
...
rule summaryTableCancer:
    ...
    log:
        "output/{sample}/log/summaryTable.log",
    shell:
        """
...
rule cancerFigures:
    input:
        "output/{sample}/summaryTable.tsv",
    output:
        "output/{sample}/figures/aseCause.pdf",
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/cancerFigures.log",
    shell:
        """
...
rule karyogram:
    ...
    container:
        "docker://glenn032787/ase_rcontainer:2.0"
    log:
        "output/{sample}/log/karyogram.log",
    shell:
        """
